...
DgContext captureFromNode(const MObject& node) {
    ...
    try {
        ...
        // 리뷰 Finding 2: 노드 이름과 타입을 원했지만 둘 다 얻지 못한
        // 경우다(손상된 노드라 조회 자체가 실패) -- "이 자리에 관여가
        // 없었다"는 빈 문자열과 구분해야 패널이 "정보 없음"과 "도구 고장"을
        // 헷갈리지 않는다. 생성자가 던졌으므로 nodeType도 axisOrTarget도
        // 얻지 못한 채 함께 비었다 -- 둘 다 표시한다.
        ctx.nameUnavailable = true;
        ctx.typeUnavailable = true;
    }
    return ctx;
}
