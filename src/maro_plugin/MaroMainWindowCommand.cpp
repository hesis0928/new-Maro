...
MStatus MaroMainWindowCommand::doIt(const MArgList& /*args*/) {
    // 컨트롤 이름("maroMainWindowControl")과 뷰포트 이름 두 개
    // ("maroMainWindowViewportMaya"/"maroMainWindowViewportRos", Phase 2부터
    // 하나에서 둘로 늘어남)는 python/maroMainWindow.py가 정하고,
    // MaroPluginMain.cpp의 uninitializePlugin이 언로드할 때 그 세 이름을
    // MEL로 다시 부른다 -- 세 곳(이 파일, MaroPluginMain.cpp,
    // maroMainWindow.py)이 같은 문자열에 묶여 있으므로
