...
//
// 리뷰 Finding C1(리브니스) -- 파일을 나누는 것으로 닫힌 것과 닫히지 않은 것:
// 위 문단이 닫은 것은 **오귀속**이다(한쪽의 close 줄이 다른 쪽 세션에 붙는
// 일, 그리고 트렁케이트 경합). 닫지 못한 것이 하나 남아 있었다: 파일 내용만
// 보면 "아직 close를 안 썼다(도는 중)"와 "끝내 close를 못 썼다(크래시)"가
// 완전히 같은 모양이라는 것. 파일을 나눠도 이 둘은 여전히 구분되지 않는다 --
// 오히려 파일을 나눴기 때문에 남의 파일을 읽는 일이 생기면서 표면화됐다:
// 두 번째 Maya 창이 뜨면 그 openJournal()이 첫 번째 창의(멀쩡히 살아 있는)
// 파일을 읽고 그 미종료 세션을 크래시로 센다. 그래서 집계 쪽은 파일 이름에
// 박힌 pid가 아직 살아 있는지를 파일 밖에 물어본 뒤, 살아 있으면 그 파일의
// 마지막 미종료 세션을 세지 않는다(JournalReader::
// countCrashAdjacencyAcrossJournalFiles, ProcessLivenessFn). 여전히 닫히지
// 않은 채로 남는 것: pid 재사용(죽은 pid를 물려받은 다른 산 프로세스) --
// 그때는 진짜 크래시 하나를 놓치는 쪽으로 틀리며, 그 방향을 일부러 골랐다.
