...
| 3-1. Maro 메뉴 (멱등성 포함) | 0-1 | 필수 (Task 3) | **PASS** | 2026-08-25 사용자 / 멱등성은 2026-09-10 자동화 | 메뉴 표시/클릭은 사용자가 확인. 멱등성은 더 이상 수동 항목이 아니다 -- `tests/maya/test_main_menu.py`가 `maroMenu.cmds`를 스텁해 가드 분기를 직접 검증한다(메뉴가 이미 있으면 0개 생성, 없으면 9개). 배치에서 `cmds.menu(exists=True)`가 항상 False라 실제 호출로는 못 타던 분기다 |
