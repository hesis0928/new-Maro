...
- [ ] **[go/no-go] 픽셀 부호/방향 규약 확정 (X, Y 둘 다 실측 확인됨)** —
      **Task 4 구현 중 배치 렌더로 `xCam` 실측 확인, 이후 리뷰에서 지적된
      ...
      다소 부정확하게 유도하는, `unprojectDepthToPoints()`의 부호와는
      무관한 기존 정밀도 특성 때문으로 진단됨(자세한 근거는
      `arnold-task-4-report.md`의 2026-09-04 addendum 참고, 이번 작업
      범위 밖이라 고치지 않음). **결론: `xCam`/`yCam` 부호 둘 다 이미
      올바르다 — 수정하지 않았다.** 이 항목은 회귀 확인용으로 체크리스트에
