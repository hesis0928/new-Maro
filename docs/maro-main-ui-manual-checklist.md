...
- [ ] **[go/no-go] 픽셀 부호/방향 규약 확정** — **Task 4 구현 중 같은
      배치 렌더로 이미 실측 확인됨** — 위 평면 앞쪽(카메라에 더 가까운
      `Z=-425`)에 world `X=+100`(카메라 기준 오른쪽)에 폭 50 큐브를
      두고 렌더 → `unprojectDepthToPoints()`로 역투영한 결과, 큐브
      앞면에 해당하는 점들(월드 Z가 -430~-420 사이인 점, 1454개)의
      월드 X 범위가 `[73.7, 125.0]`, 평균 `99.5`로 나와 실제 배치
      위치(X=75~125, 중심 100)와 정확히 일치했다 — 부호가 뒤집히지
      않았다. `xCam`/`yCam` 부호는 **이미 올바르다 — 수정하지 않았다.**
      이 항목도 회귀 확인용으로 남긴다: 카메라 정면에서 한쪽으로 치우친
      위치(카메라 기준 오른쪽)에 오브젝트를 두고 렌더 → 역투영된
      포인트클라우드에서 그 오브젝트가 실제로 같은 쪽(오른쪽)에
...
> **검증 상태(Task 4, 2026-09-04):** 위 두 go/no-go 항목(Depth AOV 규약,
> 픽셀 부호)은 이 태스크를 구현한 환경에 batch `mayapy` + 실제 라이선스된
