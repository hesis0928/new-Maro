...
> 것까지 확인). 아래 1~3번 항목은 이 테스트가 대체하므로 수동으로 다시 볼
> 필요가 없다 — 남는 것은 **실제 뷰포트에서 눈으로 보이는지**와
> **그려지는 중에 언로드해도 안전한지**뿐이다.
>
...
| 6-1. `maroPointCloud` 점이 뷰포트에 보인다 (기본 크기/색) | 5(LiDAR) | 필수 | **FAIL** | 2026-09-06 / 사용자 / Maya 2026 | 점이 전혀 안 보임(선택 하이라이트조차 없음). 같은 뷰포트의 일반 로케이터는 정상 표시 확인 — 뷰포트/렌더러 문제 아님. 위 CRITICAL 경고 참고 |
| 6-1. `pointSize` 변경이 즉시 반영된다 | 5(LiDAR) | 필수 | **BLOCKED** | 2026-09-06 | 위 항목이 FAIL이라 확인 불가 |
| 6-1. `enabled` 토글이 반영된다 | 5(LiDAR) | 관찰 | **BLOCKED** | 2026-09-06 | 위 항목이 FAIL이라 확인 불가 |
| 6-1. **`.points`를 다른 배열로 재설정 — 그려진 점이 실제로 바뀐다** | 5(LiDAR) | 필수 | **BLOCKED** | 2026-09-06 | 위 항목이 FAIL이라 확인 불가 |
| 6-1. **점이 보이는 상태로 언로드 — 무크래시** | 5(LiDAR) | 필수 | **FAIL (별개 발견)** | 2026-09-06 / 사용자 / Maya 2026 | `createNode("maroPointCloud")` 단독 호출만으로도 Maya가 반복적으로 크래시함(언로드까지 가지도 못함) — Task 1의 진짜 go/no-go가 훨씬 이전 단계에서 이미 실패 |
