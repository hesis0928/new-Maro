...
타겟 메쉬 확인 → 실제 추출/캐스팅"** 순으로 바꾼다. 반환값의 의미(`LidarScanResult`
각 항목)는 그대로다 — 계산 순서만 바뀐다.
...
- 상태별 스킵 범위는 균일하지 않다 — 검사 2/3(범위 밖/FOV 밖)은 실제 스캔
  ...
  - `kMeshExtractFailed`(연결된 메쉬는 있으나 전부 추출 실패, §3.3): 행렬/
    range/FOV는 여전히 유효하므로 검사 2/3은 정상 수행하고, 검사 1/4만
    스킵(히트점 자체가 없음).
