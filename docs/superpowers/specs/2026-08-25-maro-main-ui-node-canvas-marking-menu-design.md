...
- **검사/기술 진단 터미널(신규 "테크 Diag")** — 뷰포트 하단에 기구학/물리적 타당성 검사, 센서 커버리지 분석, LiDAR 레이캐스팅 정합성 검증, 선택적 자동 보정을 제공하는 별도 서브시스템. **기존 `boad`/`book` 디버깅용 Diag(`2026-08-14-maro-troubleshooting-ecosystem-design.md`)와는 목적이 다르다** — 그쪽은 플러그인/Maya 크래시에 대비하는 백그라운드 디버깅 유틸리티이고, 이 신규 Diag는 "로봇화된 오브젝트"가 실제로 기구학적/물리적으로 말이 되는 움직임을 내는지를 MaroUI 안에서 능동적으로(버튼으로 트리거) 검사하는 완전히 별개의 기술 검증 시스템이다. 사용자 결정에 따라 이 스펙과 분리해 별도로 브레인스토밍한다 — S4(LiDAR 시각화)/S5(URDF 내보내기)를 로드맵에서 분리했던 것과 같은 이유다. 이 문서가 정한 MaroUI 레이아웃(§2, §6)은 그 서브시스템이 나중에 뷰포트 하단 어딘가에 추가될 여지를 남겨 두지만, 정확한 배치는 그 별도 스펙이 정한다.
