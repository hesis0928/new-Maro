...
- **우클릭 후 그 자리에서 바로 릴리즈**(드래그 없음, 클릭에 가까움) → 그 정확한 좌표에 바인딩 안 된 새 축(`maroAxis` 신규 생성) + 빈 `undefined` 노드가 생긴다.
...
- 바인딩 안 된 축 → `Bind`(현재 씬 선택을 타겟으로 `maroBindAxis(axis, selection[0])` 호출)
