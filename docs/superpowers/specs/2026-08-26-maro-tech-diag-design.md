...
**서로 조상/자손 관계인 축 쌍은 전부 제외한다**: `maroAxis`는 `parentAxis`로 명시적 축 체인을 갖고, `exactWorldBoundingBox()`는 그 트랜스폼의 DAG 자손을 **전부**(직속 자식 하나가 아니라) 포함한다. 링크 메쉬를 DAG로 중첩하는 흔한 리깅에서는, 예를 들어 조부모→부모→자식 3단 체인이라면 조부모 링크의 월드 AABB가 부모 링크뿐 아니라 자식 링크까지 통째로 품는다 — 이 잡음은 인접(직속 부모-자식) 쌍에만 국한되지 않고 체인 전체를 가로지른다. 그래서 인접 쌍만 걸러내면 조부모-자손처럼 한 단계 이상 건너뛴 조상/자손 쌍은 그대로 남아 여전히 가짜 충돌을 낸다. 겹침 판정 자체("박스를 주면 겹치는 것을 찾는다")는 그대로 두고, 씬의 축 부모 관계를 아는 호출자 쪽에서 각 축의 `parentAxisPath` 체인을 뿌리까지 전부 따라가(`adjacentMeshPairs()`) 조상/자손 관계에 있는 모든 축 쌍의 메쉬를 결과에서 걸러낸다.
