...
- 관절 원점(`<origin>`)은 축 로케이터 자신의 실제 월드 트랜스폼에서 가져온다. **정정(전체 브랜치 최종 리뷰에서 발견, 실측 반증됨)**: 관절 축(`<axis>`)을 `conventionAxis`가 가리키는 로케이터의 로컬 축(X=(1,0,0)/Y=(0,1,0)/Z=(0,0,1))에 별도 변환 없이 그대로 매핑한다는 이 문단의 원래 주장은 틀렸다 — `maroMayaToRos`의 실제 변환(`(x,y,z)->(x,-z,y)`)은 X축만 그대로 두고 Y/Z를 서로 바꾸므로, `conventionAxis`가 가리키는 벡터에도 같은 재배치를 적용해야 한다(맞는 매핑: X→(1,0,0), Y→(0,0,1), Z→(0,-1,0)). 기본값인 conventionAxis=Y가 실제로 틀린 축을 내보내고 있었다 — 자세한 근거는 설계 스펙 §3.3 정정, 수정된 구현은 Task 4 "Fix Round 2" 커밋 참고. 이 정정 이전에 적힌 이 문서의 Task 1/Task 4 Step 1 안 코드 블록(아래)은 이 두 번의 수정 라운드(capMin/capMax 단위, axis 변환) 이전 버전이라 최종 구현과 다르다 — 실제 동작은 항상 `git log`/최신 `python/maroUrdfExport.py` 소스가 우선한다.
...
- `capabilityIn[].capMin`/`capMax`는 (정정: 이 문단의 이전 버전은 틀렸었다) `MFnUnitAttribute`가 아니라 **평범한 `MFnNumericData::k3Double`**이다(`MaroAxisNode.cpp`) — `cmds.getAttr`은 이 값에 UI 단위 변환을 절대 적용하지 않으며, `cmds.currentUnit()`과 무관하게 항상 데이터블록에 저장된 그대로(생 raw)의 값이 나온다. `MaroCapabilityNodes.cpp`의 `MaroLimitNode`/`MaroTranslationLimitNode::compute`가 자기 쪽 진짜 unit-typed 속성을 이 평범한 double 컴파운드에 쓰기 **전에** 이미 고정 단위로 변환해 둔다 — capType 1(rotation limit)의 `capMin`/`capMax`는 항상 **라디안**(`.asRadians()` 후 저장), capType 5(translation limit)는 항상 **센티미터**(`.asCentimeters()` 후 저장), 세션의 각도/선형 단위 설정과 무관하게 항상 그렇다. 따라서 순수 함수에 넘기기 전 처리는: capType 1은 `cmds.getAttr` 값을 그대로 쓴다(`MAngle.uiUnit()`으로 감싸면 안 된다 — 그 감싸기 자체가 버그다). capType 5는 `MDistance(raw, MDistance.kCentimeters).asMeters()`처럼 **고정** 소스 단위로만 변환한다(`.uiUnit()`을 쓰면 안 된다). 이 정정은 `python/maroTechDiag.py`가 이미 올바르게 해결해 둔 것과 정확히 같은 결론이다(`capMin`/`capMax`를 순수 `cmds.getAttr`로만 읽는다, `2026-08-26-maro-tech-diag-design.md` §3.1) — 이 문서의 이전 버전은 그 선례를 반대 방향으로 잘못 읽어 Task 4에 실제 버그(도(degree) 기본 단위 세션에서 라디안 한계값이 ~57배 어긋남)를 만들었었다.
